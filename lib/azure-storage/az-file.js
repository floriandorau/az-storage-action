...
const createTargetFileName = (file, version) => {
    const fileParts = path.parse(file);
    return `${fileParts.name}_${version}${fileParts.ext}`;
};

const uploadFile = (directory, file, version) => {
    const fileStats = fs.statSync(file);
    const fileStream = fs.createReadStream(file);

    const fileName = createTargetFileName(file, version);
...
exports.upload = async (file, version, { account, accountKey, containerName }) => {
    ...
    await uploadFile(share, file, version);
